...
struct std::hash<ShashChess::FixedString<N>> {
    std::size_t operator()(const ShashChess::FixedString<N>& fstr) const noexcept {
        return ShashChess::hash_bytes(fstr.data(), fstr.size());
    }
};
