...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
	...
	if (( // from Crystal begin
		...
		&& (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 17139 && eval >= beta && eval >= ss->staticEval && ss->staticEval >= beta - 20 * depth - improvement / 13 + 233 + complexity / 25 && pos.non_pawn_material(us) && ((pos.this_thread()->shashinQuiescentCapablancaMaxScore) || (!gameCycle)) // from Crystal
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || (cutNode && (ss - 1)->moveCount > 1) || (bestValue >= VALUE_TB_WIN_IN_MAX_PLY))) // lmr_mate
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              //by Shashin begin
              newDepth += doDeeperSearch +((pos.this_thread()->shashinQuiescentCapablancaMaxScore)?- doShallowerSearch:0) + 
              ((pos.this_thread()->shashinValue!=SHASHIN_POSITION_CAPABLANCA)?doEvenDeeperSearch:0); 
              //by Shashin end
    ...
    */
    ...
    else if (   (depth >= 5 || (PvNode && pos.this_thread()->shashinQuiescentCapablancaMiddleHighScore) || bestValue < alpha - 62 * depth)  // official with Shashin
             && !priorCapture)
    {
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (  !PvNode
        ...
        && tte->depth() >= ttDepth
        && ttValue != VALUE_NONE // Only in case of TT access race                                             // Only in case of TT access race
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
        return ttValue;
