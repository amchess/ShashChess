...
std::vector<LearningFileEntry> gameLine;//Kelly
//kelly end
namespace Search {
...
void Search::clear() {
  ...
  g_inBook = Options["Live Book Retry"];//livebook
	...
	if (bestThread->completedDepth > 4 && !pauseExperience && !Options["Read only learning"])//from Khalid
	  {
	    ...
	    {
			currentLearningEntry.score = makeExpValue(currentLearningEntry);
			gameLine.push_back(currentLearningEntry);
	    }
	    else
	    {
...
void Thread::search() {
  ...
  double floatLevel = limitStrength ?
                        Utility::clamp(std::pow((uciElo - 1346.6) / 143.4, 1 / 0.806), 0.0, 20.0) :
                        double(skillLevel);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    Piece movedPiece;
    int moveCount, captureCount, quietCount, rootDepth; //from Crystal
    //from Kelly begin
    ...
    if(persistedLearning)
    {
        ...
        {
            NodeInfo* node = getNodeFromHT(posKey);
        	if (node != nullptr)
			{
				MoveInfo moveInfo = node->latestMoveInfo;
	            sibs = node->siblingMoveInfo.size();
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
	  ...
	  //goodStatic6 begin
              ...
              && !givesCheck)
          {
              ...
              //SPRT histSeeQ2  begin
	      int counterHist = ((*contHist[0])[movedPiece][to_sq(move)] +
		     (*contHist[1])[movedPiece][to_sq(move)]) / 8192;
								 
              // Prune moves with negative SEE (~20 Elo)
              if (!pos.see_ge(move, Value(-(29 + counterHist - std::min(lmrDepth, 18)) * lmrDepth * lmrDepth))) //SPRT histSeeQ2 end
                  continue;
          }
          else
          {
	    ...
	    if(pos.this_thread()->shashinQuiescentCapablancaMaxScore)
	    {
	      ...
	      if (   !givesCheck //Introduce capture history pruning official adapted with Shashchess
		       && lmrDepth < 1
	  ...
	  &&  moveCount > 1 + ((pos.this_thread()->shashinValue==SHASHIN_POSITION_CAPABLANCA)? (2 * rootNode + 2 * (PvNode && abs(bestValue) < 2)):
	                             (msb(std::max(1, (thisThread->rootDepth - 2 * ss->ply) / 8))))//LMRRootDepth
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    bool ttHit, pvHit, givesCheck, captureOrPromotion, gameCycle; //from Crystal
    int moveCount;
...
void putGameLineIntoLearningTable()
{
  ...
  if (gameLine.size() > 1)
  {
    for (size_t index = gameLine.size() - 1; index > 0; index--)
    {
      ...
      insertIntoOrUpdateLearningTable(gameLine[index - 1]);
    }
