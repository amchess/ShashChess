...
// Live Book begin
        ...
        if (int(Options["MultiPV"]) == 1 && !depthLimit
...
// livebook begin
    ...
    inline int getShashinValue(Value score)
    {
        ...
        if (((int)score >= -SHASHIN_TAL_THRESHOLD) && ((int)score <= -SHASHIN_CAPABLANCA_THRESHOLD))
        {
        ...
        if (((int)score >= SHASHIN_CAPABLANCA_THRESHOLD) && ((int)score <= SHASHIN_TAL_THRESHOLD))
        {
    ...
    void Thread::search()
    {
        ...
        else
        {
            ...
            while (++rootDepth < MAX_PLY // handicap mode
                   && !Threads.stop && !(depthLimit && mainThread && rootDepth > depthLimit))
            {
                ...
                {
                    double fallingEval = (71 + 12 * (mainThread->bestPreviousAverageScore - bestValue) + 6 * (mainThread->iterValue[iterIdx] - bestValue)) / 656.7;
                    ...
                    else if (Threads.increaseDepth && !mainThread->ponder && Time.elapsed() > totalTime * 0.53)
    ...
    namespace
    {
        ...
        Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth, bool cutNode)
        {
            ...
            if (!PvNode && ss->ttHit && ((!gameCycle) || (pos.this_thread()->shashinQuiescentCapablancaMaxScore)) // from Crystal
                && tte->depth() > depth - (tte->bound() == BOUND_EXACT) && ttValue != VALUE_NONE                  // Possible in case of TT access race
            ...
            if ( // from Crystal begin
                ...
                // from Crystal end
                && depth < 8 && eval - futility_margin(depth, improving) - (ss - 1)->statScore / 303 >= beta && eval >= beta && eval < 28031) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
                return eval;

            // Step 9. Null move search with verification search (~22 Elo)
            if (
                ( // from Crystal begin
                    ((pos.this_thread()->shashinQuiescentCapablancaMiddleHighScore && !PvNode && !excludedMove) ||
                     (!pos.this_thread()->shashinQuiescentCapablancaMiddleHighScore && !kingDanger))
                    // from Crystal end
                    )
                // from Crystal begin
                ...
                // from Crystal end
                && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor) && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 17139 && eval >= beta && eval >= ss->staticEval && ss->staticEval >= beta - 20 * depth - improvement / 13 + 233 + complexity / 25 && pos.non_pawn_material(us) && ((pos.this_thread()->shashinQuiescentCapablancaMaxScore) || (!gameCycle)) // from Crystal
        ...
        moves_loop: // When in check, search starts here
            ...
            if (ss->inCheck && !PvNode && depth >= 2 && ttCapture && (tte->bound() & BOUND_LOWER) && tte->depth() >= depth - 3 && ttValue >= probCutBeta && abs(ttValue) <= VALUE_KNOWN_WIN && abs(beta) <= VALUE_KNOWN_WIN)
                return probCutBeta;
            ...
            while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
            {
                ...
                { // end from Crystal
                        ...
                        && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
                    {
                        ...
                        if (capture || givesCheck)
                        {
                            ...
                            if (!givesCheck && !PvNode && lmrDepth < 7 && !ss->inCheck && ss->staticEval + 180 + 201 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))] + captureHistory[movedPiece][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] / 6 < alpha)
                                continue;
                        ...
                        {
                            int history = (*contHist[0])[movedPiece][to_sq(move)] + (*contHist[1])[movedPiece][to_sq(move)] + (*contHist[3])[movedPiece][to_sq(move)];
                    ...
                    if (ss->ply < thisThread->rootDepth * 2)
                    {
                        ...
                        if (!rootNode && depth >= 4 - (thisThread->previousDepth > 24) + 2 * (PvNode && tte->is_pv()) && move == ttMove && !excludedMove // Avoid recursive singular search
                                                                                                                                                         /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
                            && abs(ttValue) < VALUE_KNOWN_WIN && (tte->bound() & BOUND_LOWER) && tte->depth() >= depth - 3)
                        {
                            ...
                            else if (ttValue >= beta && (((ss - 1)->moveCount > 1) && (!gameCycle))) // from Crystal
                        ...
                        else if (givesCheck && depth > 9 && abs(ss->staticEval) > 82)
                            extension = 1;

                        // Quiet ttMove extensions (~0 Elo)
                        else if (PvNode && move == ttMove && move == ss->killers[0] && (*contHist[0])[movedPiece][to_sq(move)] >= 5177)
                            extension = 1;
                    }
                ...
                skipExtensionAndPruning: // full threads search patch
                    ...
                    //to integrate official begin
                    bool skipFullDepthSearch = false;
                    bool updatedNewDepth = false;
                    //to integrate official end
                    ...
                    if (doLMRStep && depth >= 2 && moveCount > sibs                                                                                                               // full threads patch + Kelly
                        && moveCount > 1 + (PvNode && ss->ply <= 1) && (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1) || (bestValue >= VALUE_TB_WIN_IN_MAX_PLY))) // lmr_mate
                    {
                        ...
                        if (cutNode)
                        {
                            ...
                            {
                                r += std::clamp(!ss->ttPv - ss->ttPv + !capture - capture + (move != ss->killers[0]) - (move == ss->killers[0]), 1, complexity > 650 ? 2 : 3);
                            }
                        }
                        ...
                        if (PvNode && !ss->inCheck && abs(ss->staticEval - bestValue) > 250 && (!(pos.this_thread()->shashinQuiescentCapablancaMaxScore)))
                            r--;
                        ...
                        if ((ss + 1)->cutoffCnt > 3 && (pos.this_thread()->shashinQuiescentCapablancaMaxScore))
                            r++;
                        // official with Shashin end
                        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)] + (*contHist[0])[movedPiece][to_sq(move)] + (*contHist[1])[movedPiece][to_sq(move)] + (*contHist[3])[movedPiece][to_sq(move)] - 4433;

                        // official by Shashin begin
                        //  Decrease/increase reduction for moves with a good/bad history (~30 Elo)
                        r -= ss->statScore / (13628 + 4000 * ((depth > 7 && depth < 19) && ((((ss->statScore) > 0) && (!(pos.this_thread()->shashinQuiescentCapablancaMaxScore))) ||
                        ...
                        {
                            doShallowerSearch = value < bestValue + (2 - (ss->ttPv && !PvNode)) * newDepth;//do_shallow_former_pv_00
                            newDepth += doDeeperSearch - doShallowerSearch;
                            updatedNewDepth=true;
                            if (newDepth <= d)
                            {
                                skipFullDepthSearch = true;
                            }
                        }
                        // official by Shashin end
                        didLMR = true;
                    ...
                    else
                    {
                        ...
                        // deeperPvS12 begin
                        if (!(((pos.this_thread()->shashinValue == SHASHIN_POSITION_TAL) && (!(pos.this_thread()->shashinQuiescentCapablancaMiddleHighScore)) && (pos.this_thread()->shashinQuiescentCapablancaMaxScore)) ||
                              ...
                              ((pos.this_thread()->shashinValue == SHASHIN_POSITION_PETROSIAN) && (pos.this_thread()->shashinQuiescentCapablancaMiddleHighScore))))
                        {
                            ...
                            skipFullDepthSearch = false;
                            updatedNewDepth=false;
                        }
                        // deeperPvS12 end
                        didLMR = false;
                    }
                    ...
                    if (doFullDepthSearch)
                    {
                        ...
                        {
                            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha, (updatedNewDepth? newDepth: newDepth + doDeeperSearch - doShallowerSearch), !cutNode);
                        }
                ...
                if (value > bestValue)
                {
                    ...
                    if (value > alpha)
                    {
                        ...
                        if (PvNode && value < beta) // Update alpha! Always alpha < beta
                        {
                            ...
                            if (depth > 1 && depth < 10 && beta < VALUE_KNOWN_WIN && alpha > -VALUE_KNOWN_WIN && depth >= nonReducedDepth)
                                depth -= 1;
                            assert(depth > 0);
                        }
                        else
            ...
            */
            ...
            if (!moveCount)
                bestValue = excludedMove ? alpha : ss->inCheck ? mated_in(ss->ply)
                                                               : VALUE_DRAW;
            ...
            else if ((depth >= 5 || PvNode) && !priorCapture)
            {
                ...
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, stat_bonus(depth) * (1 + ((pos.this_thread()->shashinValue == SHASHIN_POSITION_CAPABLANCA) ? (extraBonus) : 0))); // official with Shashin
            }
            ...
            if (!excludedMove && !(rootNode && thisThread->pvIdx))
                tte->save(posKey, value_to_tt(bestValue, ss->ply), ss->ttPv,
                          bestValue >= beta ? BOUND_LOWER : PvNode && bestMove ? BOUND_EXACT
                                                                               : BOUND_UPPER,
        ...
        Value qsearch(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth)
        {
            ...
            if (!PvNode && ss->ttHit && ((pos.this_thread()->shashinQuiescentCapablancaMaxScore) || (!gameCycle)) // from Crystal
                && tte->depth() >= ttDepth && ttValue != VALUE_NONE                                               // Only in case of TT access race
                && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
                return ttValue;
            ...
            while ((move = mp.next_move()) != MOVE_NONE)
            {
                ...
                if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && !givesCheck && to_sq(move) != prevSq && futilityBase > -VALUE_KNOWN_WIN && type_of(move) != PROMOTION)
                {
                ...
                // Continuation history based pruning (~2 Elo)
                if (!capture && bestValue > VALUE_TB_LOSS_IN_MAX_PLY && (*contHist[0])[pos.moved_piece(move)][to_sq(move)] < 0 && (*contHist[1])[pos.moved_piece(move)][to_sq(move)] < 0)
                    continue;
                              ...
                              Move *quietsSearched, int quietCount, Move *capturesSearched, int captureCount, Depth depth)
        {
            ...
            if (((ss - 1)->moveCount == 1 + (ss - 1)->ttHit || ((ss - 1)->currentMove == (ss - 1)->killers[0])) && !pos.captured_piece())
    ...
    void MainThread::check_time()
    {
        ...
        if ((Limits.use_time_management() && (elapsed > Time.maximum() - 10 || stopOnPonderhit)) || (Limits.movetime && elapsed >= Limits.movetime) || (Limits.nodes && Threads.nodes_searched() >= (uint64_t)Limits.nodes))
            Threads.stop = true;
    }
