...
#undef S
  ...
  Score Evaluation<T>::king() const {
    ...
    //QRcheckOverpal1 begin
    queenChecks =  (b1 | b2)
                 & attackedBy[Them][QUEEN]
                 & safe
                 & ~attackedBy[Us][QUEEN];
    if (queenChecks & ~rookChecks)
	kingDanger += more_than_one(queenChecks & ~rookChecks) ? SafeCheck[QUEEN][1] :
		      more_than_one(queenChecks) ? SafeCheck[QUEEN][0] * 5/4
		      : SafeCheck[QUEEN][0];
    //QRcheckOverpal1 end
  ...
  Value Evaluation<T>::winnable(Score score) const {
    ...
    int complexity =   9 * pe->passed_count()
                    ...
                    - 43 * almostUnwinnable
		    		+(pos.this_thread()->shashinValue!=SHASHIN_POSITION_CAPABLANCA)?0:(- 2 * pos.rule50_count()) //official integrated with Shashin
                    -110 ;
...
make_v:
    ...
    //begin official integrated by Shashin
    ...
    return v;
    //end official integrated by Shashin
  }
...
Value Eval::evaluate(const Position& pos) {
  ...
  Value v = (Eval::useNNUE) ? NNUE::evaluate(pos) * 5 / 4 + Tempo:Evaluation<NO_TRACE>(pos).value();//only pure-classical
