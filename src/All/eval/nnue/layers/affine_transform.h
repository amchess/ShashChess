  ...
  namespace NNUE {
    namespace Layers {
      ...
      public:
        ...
        static constexpr std::size_t kSelfBufferSize = CeilToMultiple(kOutputDimensions * sizeof(OutputType), kCacheLineSize);
        ...
        static std::string GetStructureString() {
          return "AffineTransform[" + std::to_string(kOutputDimensions) + "<-"
        ...
        bool WriteParameters(std::ostream& stream) const {
          ...
          stream.write(reinterpret_cast<const char*>(weights_), kOutputDimensions * kPaddedInputDimensions * sizeof(WeightType));
        ...
        const OutputType* Propagate(const TransformedFeatureType* transformed_features, char* buffer) const {
          ...
          for (IndexType i = 0; i < kOutputDimensions; ++i)
          {
              ...
              {
                  __m512i product0 = _mm512_maddubs_epi16(_mm512_loadAU_si512(&input_vector[j]),
                                                          _mm512_load_si512(&row[j]));
                  product0 = _mm512_madd_epi16(product0, kOnes);
                  sum = _mm512_add_epi32(sum, product0);

                  __m512i product1 = _mm512_maddubs_epi16(_mm512_loadAU_si512(&input_vector[j+1]),
                                                          _mm512_load_si512(&row[j+1]));
              ...
              if (kNumChunks & 0x1)
              {
                  __m512i product = _mm512_maddubs_epi16(_mm512_loadAU_si512(&input_vector[kNumChunks-1]),
                                                         _mm512_load_si512(&row[kNumChunks-1]));
              ...
              if (kPaddedInputDimensions != kNumChunks * kSimdWidth * 2)
              {
                  ...
                  __m256i product256 = _mm256_maddubs_epi16(_mm256_loadAU_si256(&iv256[0]),
                                                            _mm256_load_si256(&row256[0]));
              ...
              for (int j = 0; j < (int)kNumChunks - 1; j += 2)
              {
                  __m256i product0 = _mm256_maddubs_epi16(_mm256_loadAU_si256(&input_vector[j]),
                                                          _mm256_load_si256(&row[j]));
                  product0 = _mm256_madd_epi16(product0, kOnes);
                  sum = _mm256_add_epi32(sum, product0);

                  __m256i product1 = _mm256_maddubs_epi16(_mm256_loadAU_si256(&input_vector[j+1]),
                                                          _mm256_load_si256(&row[j+1]));
              ...
              if (kNumChunks & 0x1)
              {
                  __m256i product = _mm256_maddubs_epi16(_mm256_loadAU_si256(&input_vector[kNumChunks-1]),
                                                         _mm256_load_si256(&row[kNumChunks-1]));
              ...
              for (int j = 0; j < (int)kNumChunks - 1; j += 2)
              {
                  __m128i product0 = _mm_maddubs_epi16(_mm_load_si128(&input_vector[j]),
                                                       _mm_load_si128(&row[j]));
                  product0 = _mm_madd_epi16(product0, kOnes);
                  sum = _mm_add_epi32(sum, product0);

                  __m128i product1 = _mm_maddubs_epi16(_mm_load_si128(&input_vector[j+1]),
                                                       _mm_load_si128(&row[j+1]));
              ...
              if (kNumChunks & 0x1)
              {
                  __m128i product = _mm_maddubs_epi16(_mm_load_si128(&input_vector[kNumChunks-1]),
                                                      _mm_load_si128(&row[kNumChunks-1]));
...
#endif // _NNUE_LAYERS_AFFINE_TRANSFORM_H_
