...
### Use NNUE
    Default is Hybrid: the same behaviour as Stockfish NNUE. The other values are "Pure" (only the net and not classical evaluation at all: useful, especially, for some hard positions) and classical (the evaluation not from the net at all)
